...
export default function ActiveWorkoutScreen() {
  ...
  useEffect(() => {
    return navigation.addListener('beforeRemove', (e) => {
      if (isSavingRef.current) return;           // finishing — let it through
      if (exercisesRef.current.length === 0) return; // nothing entered yet
      e.preventDefault();
      Alert.alert(
        'Discard Workout?',
        'Going back will discard all unsaved exercises.',
        [
  ...
  const finishWorkout = useCallback(async () => {
    ...
    if (session.exercises.length === 0) {
      Alert.alert('No Sets Logged', 'Enter at least one set before finishing.');
      return;
    }
    isSavingRef.current = true;
    await save(session);
    navigation.dispatch(StackActions.popToTop()); // clears stack so back ≠ editing screen
  }, [save, navigation]);

  // ── Header (back + finish buttons) ────────────────────────────────────────
  useEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => (
        <TouchableOpacity onPress={finishWorkout} style={{ marginRight: 4 }}>
          <Text style={{ color: colors.accent, fontWeight: '700', fontSize: 16 }}>Finish</Text>
        </TouchableOpacity>
      ),
    });
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        {exercises.length === 0 && (
          <View style={styles.placeholder}>
            ...
            <Text style={styles.placeholderText}>Tap "Add Exercise" to get started</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  // Placeholder
  placeholder: { alignItems: 'center', paddingVertical: 48, gap: 12 },
